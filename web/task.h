...
char *path = (char *) "/Users/liushiming/CLionProjects/project4cpp/web";
...
void task::response_get(char *filename)
{
    ...
    } else
    {
        ...
        // 第 1 个参数 fd，是一个要拷贝文件的文件描述符
        // 第 2 个参数 s，必须指向一个要输出数据 的 socket 。。
        // 第 3 个参数 offset, 是开始de偏移量。
        // 第 4 个参数 len，表示要传送的字节数，同时这也是一个结果集，会返回传送的字符集的字节数。
